...
public class DatabaseChangelog {
    ...
    @ChangeSet(order = "000", id = "initialUsersData", author = "")
    ...
    public void initialAwardsDataOnDev(MongoTemplate mongoTemplate, Environment environment) {
        Optional.ofNullable(environment.getProperty("awards.filePath"))
                .ifPresent(path -> {
                    Optional.ofNullable(ClassLoader.getSystemClassLoader().getResource(path)).ifPresent(url -> {
                        try {
    ...
    private String getJson(URL url) throws URISyntaxException {
